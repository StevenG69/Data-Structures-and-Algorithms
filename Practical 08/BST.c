// Binary Search Tree ADT implementation ... COMP9024 25T1
...
/*
 * Rebuilds a BST in height-balanced tree using Traversal + AVL insertions.
 */
Tree rebalance(Tree t)
{
    ...
    // Allocate array for in-order keys
    Item *keys = malloc(n * sizeof(Item));
    if (!keys)
    {
        return t; // leave tree unchanged
    }

    // Traversal to fill keys[]
    Tree *stack = malloc(n * sizeof(Tree));
    int top = -1;
    Tree curr = t;
    int count = 0;
    ...
    while (curr || top >= 0)
    {
        ...
        // visit node
        curr = stack[top--];
        keys[count++] = curr->data;
        // traverse right subtree
        curr = curr->right;
    }
    free(stack);

    // Build new balanced tree in AVL inserts
    Tree balanced = NULL;
    for (int i = 0; i < n; i++)
    {
        balanced = insertAVL(balanced, keys[i]);
    }
